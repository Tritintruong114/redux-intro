...
const RootComponent = (props) => {
  ...
  const removeProductFromCart = (newProduct) => {
    ...
    const removeFromCart = cart.products.map((product) => {
      if (newProduct.id === product.id && product.qty > 0) {
        product.qty -= 1;
        product.price -= newProduct.price;
    ...
    setCart({ products: removeFromCart, totalPrice: newTotalPrice });
  };
  // Step 3
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item sm={6}>
          <ProductOne
            ...
            removeProductFromCart={props.removeFromCart}
